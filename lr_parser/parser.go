...
func (p *Parser) PrintItemSets(filename string) error {
	...
	for i, set := range p.ItemSets {
		...
		for _, symbol := range symbols {
			...
			nextIndex := p.findItemSetIndex(nextSet)
			if nextIndex != -1 {
				transitions[i][symbol] = nextIndex
			}
		}
	}

	fmt.Fprintln(file, "digraph ItemSets {")
	fmt.Fprintln(file, "    rankdir=LR;")
	fmt.Fprintln(file, "    node [shape=rectangle];")

	// 定义节点
	for i, set := range p.ItemSets {
		var productions []string
		productionsMap := make(map[string][]string)
		...
		for _, item := range set.Items {
			...
			productionsMap[dottedProd] = append(productionsMap[dottedProd], item.Lookahead)
		}

		for prod, lookaheads := range productionsMap {
			productions = append(productions, fmt.Sprintf("%s, 《%s》", prod, strings.Join(lookaheads, ", ")))
		}

		label := fmt.Sprintf("I%d:\\n%s", i, strings.Join(productions, "\\n"))
		fmt.Fprintf(file, "    I%d [label=\"%s\"];\n", i, label)
	}

	// 使用计算好的transitions定义边
	for from, symbolMap := range transitions {
		for symbol, to := range symbolMap {
...
// 打印分析表为CSV格式
func (p *Parser) PrintParsingTableCSV(filename string) error {
	file, err := os.Create(filename)
	if err != nil {
		return err
	}
	...
	terminals["$"] = true // 添加���束符号

	// 转换为有序切片
	terminalsList := make([]string, 0, len(terminals))
	nonTerminalsList := make([]string, 0, len(nonTerminals))

	for t := range terminals {
		terminalsList = append(terminalsList, t)
	}
	for nt := range nonTerminals {
		nonTerminalsList = append(nonTerminalsList, nt)
	}
	sort.Strings(terminalsList)
	sort.Strings(nonTerminalsList)

	// 写入表头
	fmt.Fprintf(file, "State")
	for _, t := range terminalsList {
		fmt.Fprintf(file, ",%s", t)
	}
	for _, nt := range nonTerminalsList {
		fmt.Fprintf(file, ",%s", nt)
	}
	fmt.Fprintln(file)
	...
	for state := 0; state <= maxState; state++ {
		fmt.Fprintf(file, "%d", state)

		// 写入Action表项
		for _, t := range terminalsList {
			fmt.Fprint(file, ",")
			if action, exists := p.Action[state][t]; exists {
				fmt.Fprint(file, action)
			}
		}
		...
		for _, nt := range nonTerminalsList {
			fmt.Fprint(file, ",")
			if nextState, exists := p.Goto[state][nt]; exists {
				fmt.Fprintf(file, "%d", nextState)
			}
		}
		fmt.Fprintln(file)
	}
